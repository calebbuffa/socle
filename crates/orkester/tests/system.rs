...
fn run_cross_context_roundtrip_stress(system: &AsyncSystem, iterations: usize, mut seed: u64) {
    for _ in 0..iterations {
        ...
        let base: orkester::Future<i32> = if should_fail {
            system.future(|promise| promise.reject("seeded failure"))
        } else {
...
fn run_shared_fanout_stress(system: &AsyncSystem, iterations: usize, waiters: usize) {
    for iteration in 0..iterations {
        let (promise, future) = system.promise::<usize>();
...
fn create_promise_pair_resolves() {
    let system = AsyncSystem::with_threads(1);
...
fn run_in_main_thread_is_inline_inside_scope() {
    ...
    assert_eq!(future.block().unwrap(), 7);
}
...
fn block_with_main_pumps_queue() {
    ...
    assert_eq!(future.block_with_main().unwrap(), 9);
}
...
fn all_future_values() {
    let system = AsyncSystem::with_threads(2);
    let futures = vec![
...
fn run_in_worker_thread_flattens_future_result() {
    ...
    let flattened: orkester::Future<i32> = system.run(orkester::Context::BACKGROUND, move || {
...
fn run_in_main_thread_flattens_future_result() {
    ...
    let flattened: orkester::Future<i32> = system.run(orkester::Context::MAIN, move || {
...
fn then_in_worker_thread_flattens_rejected_future_result() {
    ...
    let flattened: orkester::Future<i32> = system
        ...
        .then(orkester::Context::BACKGROUND, move |_| {
            system_clone.future(|promise| promise.reject("boom"))
        });
...
fn all_rejects_when_any_input_rejects() {
    ...
    let futures = vec![
        system.resolved(1_i32),
        system.future(|promise| promise.reject("join failed")),
        system.resolved(3_i32),
    ];
...
fn future_poll_deduplicates_same_waker_registration() {
    let system = AsyncSystem::with_threads(1);
    let (promise, mut future) = system.promise::<i32>();
...
fn worker_to_main_to_worker_chain_completes_with_main_pump() {
    let system = AsyncSystem::with_threads(3);
    let caller_thread = std::thread::current().id();
...
fn rejected_chain_skips_then_and_recovers_in_main_thread() {
    ...
    let failed: orkester::Future<i32> = system.run(orkester::Context::BACKGROUND, move || {
        system_clone.future(|promise| promise.reject("worker failure"))
    });
...
fn randomized_repeated_flatten_and_recovery_stress() {
    ...
    for _ in 0..ITERS {
        ...
        let outcome: orkester::Future<i32> = system.run(orkester::Context::BACKGROUND, move || {
            if should_fail {
                system_clone.future(|promise| promise.reject("random failure"))
            } else {
...
fn promise_drop_rejects_paired_future() {
    ...
    let error = future.block().unwrap_err();
...
fn all_empty_resolves_to_empty_vec() {
    let system = AsyncSystem::with_threads(1);
    let joined: orkester::Future<Vec<i32>> = system.join_all(Vec::<orkester::Future<i32>>::new());
...
fn all_preserves_input_order_when_resolved_out_of_order() {
    let system = AsyncSystem::with_threads(2);
    let (promise0, future0) = system.promise::<i32>();
    let (promise1, future1) = system.promise::<i32>();
    let (promise2, future2) = system.promise::<i32>();
...
fn catch_in_main_thread_recovers_on_main_when_pumped() {
    ...
    let recovered = system
        .future(|promise| promise.reject("main recover"))
        .catch(orkester::Context::MAIN, move |error| {
...
fn future_poll_ready_then_wait_reports_consumed() {
    let system = AsyncSystem::with_threads(1);
    let mut future = system.resolved(9_i32);
    let wake_count = Arc::new(AtomicUsize::new(0));
    let waker = make_counting_waker(Arc::clone(&wake_count));
    let mut cx = Context::from_waker(&waker);
    let mut pinned = Pin::new(&mut future);
...
fn dispatch_one_main_thread_task_reports_queue_progress() {
    ...
    for (idx, future) in futures.into_iter().enumerate() {
        assert_eq!(future.block().unwrap(), idx as i32);
    }
}
...
fn soak_shared_future_fanout_high_contention() {
    let system = AsyncSystem::with_threads(6);
